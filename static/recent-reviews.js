...
class RecentReviews extends React.Component {
    ...
    render(){
        ...
        const previousButton = <button className="btn btn-secondary" onClick={ () => {
        this.loadPrevious()} } disabled={this.state.previousUrl == null}>Previous</button>

        const nextButton = <button className="btn btn-secondary float-right" onClick={ () => {
        this.loadNext()} } disabled={this.state.nextUrl == null}>Next</button>

        let reviewItems;

        if (this.state.reviews.length === 0){
            reviewItems = <h5>No items to display.</h5>
        }
        else{
